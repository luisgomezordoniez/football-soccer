...
::: {.callout-note}
Creo que el Cleveland Plot se ve mejor así.
:::
...
Data_England$Hour=as.numeric(format(Data_England$Time, "%H"))
...
## Cambio en texto
A typical league is highly competitive, besides fighting to become champions, teams also try to qualify for European competitions or avoid relegation.


::: {.callout-note}
Me parece que podemos dejar todas las jornadas
:::
...
cuts <- sort(unique(c(1, seq(5, max_round, by = 1), max_round)))
...
ggplot(
  ...
  geom_stratum(width = 0.35, color = "grey30") +
  stat_stratum(
    geom  = "text",
...
::: {.callout-note}
Ya no tenemos odds en la tabla. Creo que se le pueden agregar más variables al PCP.
:::
...
## First option
Demasiadas líneas y no se entiende bien qué pasa con ellas. Dos opciones:
1. Solo dejar la última temporada.
2. Hacerla interactiva.
...
GGally::ggparcoord(
  ...
  scale_x_discrete(
    labels = c(
      ...
      "shots_on_target_for_s" = "Shots on Target For",
      "goals_against_s" = "Goals Against",
      "shots_on_target_against_s" = "Shots on Target Against"
    )
  ) +
  labs(
...
p <- GGally::ggparcoord(
  ...
  labs(
    title = "Team Profiles Across Seasons — Premier League",
    subtitle = "Click buttons to filter by season",
    x = "Metric",
    y = "Scaled value"
  ) +
  theme_minimal(base_size = 11) +
  ...
  layout(
    updatemenus = list(
      list(
        ...
        direction = "right",
        xanchor = "center",
        yanchor = "bottom",  # Cambié de "top" a "bottom"
        x = 0.5,
        y = -0.15,           # Cambié de 1.15 a -0.15 (negativo para ponerlo abajo)
        buttons = list(
          list(
            method = "restyle",
...
# 2. Plot
ggplot(goals_monthly, aes(x = Month, y = AvgGoals, group = League, color = League)) +
  geom_line(size = 1.2) +
  geom_point(size = 3) +
  labs(
...
```{r}
# 1. Prepare Data
cards_monthly <- Data_England %>%
  filter(Season >= 2000) %>% # Filter for reliable card data
  mutate(MatchDate = lubridate::dmy(Date),
         Month = lubridate::month(MatchDate, label = TRUE, abbr = TRUE)) %>%
  filter(!Month %in% c("Jun", "Jul")) %>%
  mutate(Month = factor(Month, levels = c("Aug", "Sep", "Oct", "Nov", "Dec", 
                                          "Jan", "Feb", "Mar", "Apr", "May"))) %>%
  group_by(Month, Season) %>%
  summarise(AvgCards = mean(HY + AY + HR + AR, na.rm = TRUE))

# 2. Plot
ggplot(cards_monthly, aes(x = Month, y = AvgCards)) +
  geom_col(fill = "#FF4B4B", alpha = 0.8) +
  facet_wrap(~Season)+
  labs(
    title = "The Winter Grind: Discipline by Month",
    subtitle = "Average Cards (Yellow + Red) per match",
    y = "Cards per Match",
    x = NULL
  ) +
  coord_cartesian(ylim = c(2.5, 4)) + # Zoom in to see differences
  theme_minimal()
...
df_big6 <- Data_England %>%
  ...
  filter(Team %in% current_tier1_teams & (!Team %in% big6)) %>%
  mutate(
